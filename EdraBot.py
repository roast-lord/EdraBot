...
sleep(2)
login = navegador.find_element_by_xpath('//*[@id="username"]')
login.send_keys(login)
senha = navegador.find_element_by_xpath('//*[@id="password"]')
...
if len(bateria) == 1:
    ...
    for j in range(0,len(marca_motor)):
        ...
        Dados = np.zeros(shape = ((delta//mass_interval+1),5))
        ...
        d[0,j] = marca_motor[j] + ' ' + modelo_motor[j] + ' - ' + bateria[z].replace('/','-') + ' - ' + diametro_helice + 'x' + passo_helice
        
        
        
        np.savetxt( 'csv/' + marca_motor[j] + ' ' + modelo_motor[j] + ' - ' + bateria[z].replace('/','-') + ' - ' + diametro_helice + 'x' + passo_helice + '.csv', Dados, delimiter=",",fmt='%1.3f')

    
    
    for x in range(0,len(marca_motor)):
        n[0,x] = marca_motor[x] + ' ' + modelo_motor[x]  
...
if len(bateria) > 1:
    for j in range(0,len(marca_motor)):
        for z in range(0,len(bateria)):
            ...
            d2[j,z] = marca_motor[j] + ' ' + modelo_motor[j] + ' - ' + bateria[z].replace('/','-') + ' - ' + diametro_helice + 'x' + passo_helice
            
            
            np.savetxt( 'csv/' + marca_motor[j] + ' ' + modelo_motor[j] + ' - ' + bateria[z].replace('/','-') + ' - ' + diametro_helice + 'x' + passo_helice + '.csv', Dados, delimiter=",",fmt='%1.3f')

        t2[0,j] = marca_motor[j] + ' ' + modelo_motor[j] + ' - ' + helice + ' ' + diametro_helice + 'x' + passo_helice + ' ' + ESC
